...
async function main() {
  ...
  if (projectInfo.internalPort) {
    console.log(`🌐 Port interne   : ${projectInfo.internalPort}`);
    console.log(`🚪 Port hôte      : ${projectInfo.hostPort} (libre et aléatoire)`);
  }

  const result = await generateFiles(projectInfo, projectPath);
  ...
  console.log(`\n🌍 Accès : http://localhost:${result.hostPort}`);
  if (result.dbPort) console.log(`🗄️  DB port : ${result.dbPort}`);
